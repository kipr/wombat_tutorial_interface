...
const importer = require("../tools/introductory-importer/import.js");
const WRITER = path.resolve("tools/introductory-importer/writer.lua");
...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), "introductory-import-"));
const first = importer.convertAll({source: SOURCE, out: temp, force: false});
assert.equal(first.written.length, 42);
assert.equal(
  fs.readdirSync(path.join(temp, "introductory", "ev3")).filter((name) => name.startsWith("project-")).length,
  17
);
assert.equal(
  fs.readdirSync(path.join(temp, "introductory", "spike")).filter((name) => name.startsWith("project-")).length,
  17
);
...
for (const platform of ["ev3", "spike"]) {
  ...
  for (let number = 1; number <= 17; number += 1) {
    ...
    const md = fs.readFileSync(
      path.join(temp, "introductory", platform, `project-${String(number).padStart(2, "0")}.md`),
      "utf8"
    );
    assert.match(md, /^---\n/, `${platform} project ${number} has front matter`);
    assert.match(md, new RegExp(`mission_id: "introductory_${platform}_coding_${String(number).padStart(2, "0")}"`));
  ...
  const hub = fs.readFileSync(path.join(temp, "introductory", platform, "_index.md"), "utf8");
  assert.match(hub, new RegExp(`platform: "${platform}"`));
  assert.match(hub, /heading_accent: "Coding"/);
  assert.ok(fs.existsSync(path.join(temp, "introductory", platform, "builds", "arm.md")));
  assert.ok(fs.existsSync(path.join(temp, "introductory", platform, "builds", "claw.md")));

  const project06 = fs.readFileSync(
    path.join(temp, "introductory", platform, "project-06.md"),
    "utf8"
  );
  assert.match(
    project06,
    new RegExp(`\\[Open the Arm Build Guide\\]\\(/introductory/${platform}/builds/arm\\) — \\*Build guide coming soon`)
  );
  assert.match(project06, new RegExp(`page: "/introductory/${platform}/builds/arm"`));
  const project07 = fs.readFileSync(
    path.join(temp, "introductory", platform, "project-07.md"),
    "utf8"
  );
  assert.match(
    project07,
    new RegExp(`\\[Open the Arm Build Guide\\]\\(/introductory/${platform}/builds/arm\\)`)
  );
  const project08 = fs.readFileSync(
    path.join(temp, "introductory", platform, "project-08.md"),
    "utf8"
  );
  assert.match(
    project08,
    new RegExp(`\\[Open the Claw Build Guide\\]\\(/introductory/${platform}/builds/claw\\)`)
  );
}
...

{
  ...
  assert.match(
    result.stdout,
    /\[Open the Arm Build Guide\]\(\/introductory\/ev3\/builds\/arm\) — \*Build guide coming soon\*/
  );
  assert.doesNotMatch(result.stdout, /GuideBuild/);
}

process.stdout.write("test_introductory_import.js: ok\n");
