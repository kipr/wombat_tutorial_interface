...
short_title: "Python 4.2"
hub_unit: 4
description: "Two tophat sensors and boolean and — each wheel watches its own sensor to straighten the robot against a line."
weight: 200
nav: python
track: python
type: labs
...
meta:
  ...
  - term: "CS1 Concepts"
    definition: "Two [[SENSOR|Sensors]] at Once · [[BOOLEAN|Boolean]] Operators (and) · Independent [[CONDITION|Conditions]]"
  - term: "Game Context"
...
- Use the boolean `and` operator to check two conditions at once.
...
You'll add these names to your [[LIBRARY|library]] as [[VARIABLE|variables]], so your code reads `k.analog(TOPHAT_LEFT)` instead of a bare number --- much easier to understand.
{{< /callout >}}
...
Each sensor controls the wheel on *its own side*: `TOPHAT_LEFT` drives the **left** wheel `k.motor(0)`, and `TOPHAT_RIGHT` drives the **right** wheel `k.motor(3)`. If these get crossed, the robot will chase the line the wrong way --- so double-check.
{{< /callout >}}
...
{{< concept "and means \"both must be true\"" >}}
- text: |
...
{{< ask key="p3_and_meaning" label="AND meaning" >}}In your own words, what does `and` do? Why is it the right operator for "stop when BOTH sensors are on the line"?{{< /ask >}}
...
{{< ask key="p7_q2_and" label="Reflection 2" n=2 >}}What does the `and` operator do, and why did the loop need it to know when to stop?{{< /ask >}}
