...
description: "Hats, stacks, and the block shelf. Timing with wait blocks, lighting the brick display, and five deliberate bugs to find and fix."
weight: 2
nav: "introductory"
mission_id: "introductory_ev3_coding_02"
mission_title: "Introductory Project 2 (EV3) — Your First Program"
...
eyebrow: "Introductory · EV3 Coding Project 2"
...
meta:
  ...
  - term: "What You Are Doing"
    definition: "Reading a word-block program piece by piece, writing your own messages on the screen, controlling timing, and learning what a bug looks like — by making five of them on purpose."
  - term: "What You Need"
    checklist:
      - key: need_1
        label: "EV3 Brick, charged"
      - key: need_2
  ...
  - term: "Before You Start"
    definition: "Project 1 — you must be able to connect to your brick, open your project, and run it."
---
...
{{< ask key="p2_guess_yellow" label="Guess about the yellow block" >}}The purple block is the only one that made something happen on the screen. What do you think the yellow one is for?{{< /ask >}}
...
| write "Hi!" | An instruction --- one action for the robot to carry out. This one writes text on the screen. The white oval is the part you can change. |
...
Every block on the colored shelf is a ready-made command --- for driving motors, drawing on the screen, playing sounds, and reading [[SENSOR|sensor]]s. You do not have to build them. The colors are the filing system:

| Color | Category |
| --- | --- |
| **Yellow** | **Events** --- hat blocks. Where programs start. |
| **Purple** | **Display** --- the brick's screen: write text, show pictures. |
...
The brick reads your stack like you read a book: top block first, then down, one at a time.

It is *fast* --- it moves from one block to the next far quicker than you can blink. Watch what that does to this program, which shows a happy face and then a sad face:

{{< wordblocks aria="Word-block stack: when program starts, display Big smile image, display Sad image" >}}
rows:
  - block:
      category: events
      shape: hat
...
Run this and you will only ever see the sad face. The happy face *was* there --- for a sliver of a second, before the next block replaced it, faster than your eye can catch.

{{< callout title="Turning Something On Is Not \"For a While\"" variant="gold" >}}
A *display image* block means "show this, starting now, until something replaces it." It does not mean "show this for a while." If you want *a while*, you have to ask for it --- and that is the **wait** block's whole job.
{{< /callout >}}

{{< wordblocks aria="Fixed stack: when program starts, display Big smile, wait 2 seconds, display Sad" >}}
rows:
  - block:
      category: events
      shape: hat
...
Connect to your brick the same way you did in Project 1 --- check your Robot Card. Open your `First Project`.
...
{{< checklist >}}
- key: p2_step_ownname
  label: "My name scrolled across the screen"
{{< /checklist >}}
...
Make a new project called `Pictures` --- names in the project name. Build the happy-face-then-sad-face program **without** the wait block first, exactly as it appeared in Learn It. Run it.

{{< ask key="p2_no_wait_result" label="Result without wait" >}}What did you see on the screen, and what did you *not* see?{{< /ask >}}
...
{{< checklist >}}
- key: p2_step_wait
  label: "The happy face held for two seconds, then the sad face appeared"
{{< /checklist >}}

{{< ask key="p2_wait_doing" label="What wait does" >}}The wait block does not show anything on the screen. What is it actually doing while the happy face is up?{{< /ask >}}
...
rows:
  ...
  - class: trial
    cells:
      - text: "Turn the brick off, then press Run in the app"
      - key: p2_bug5
        aria: "Bug 5 result"
{{< /gridtable >}}
...
- Is the brick actually connected?
...
{{< ask key="p2_read_program" label="Read the program" >}}A team builds this: hat block, sad face, wait `0.5`, happy face, wait `0.5`, sad face again. Describe exactly what the screen will do, from Run to finish.{{< /ask >}}
...
- key: p2_can_write
  label: "I can put my own words and pictures on the screen"
- key: p2_can_wait
