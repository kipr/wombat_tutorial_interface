...
description: "Lettered ports, checking direction with the mirror rule, the wheel-spin trick, and the number the brick is always counting."
weight: 3
nav: "introductory"
mission_id: "introductory_ev3_coding_03"
mission_title: "Introductory Project 3 (EV3) — Motors and Ports"
...
eyebrow: "Introductory · EV3 Coding Project 3"
...
The brick works the same way. Two motors, two wheels, two separate commands.

### Find the ports

Look at the brick. Across the **top edge** are four [[PORT|port]]s with letters --- **A, B, C, D** --- for motors. Across the **bottom edge** are four more with numbers --- **1, 2, 3, 4** --- for sensors. Letters up top, numbers down below.

{{< callout title="Any Device, Any Port" >}}
Motors get the letters --- and among the lettered ports, none is special. A motor works in any of the four, because the brick can *tell* what is plugged into each one. What matters is that your **program** and your **plugs** agree. Our convention for the driving base, from now until the end of the season: **left wheel in B, right wheel in C.**
{{< /callout >}}

{{< checklist >}}
- key: p3_step_ports
  label: "I found all eight ports and can point to motor ports A--D on top and sensor ports 1--4 on the bottom"
- key: p3_step_plugged
  label: "The left wheel motor is in port B and the right wheel motor is in port C"
- key: p3_step_block
...
You do not need a program to see this. Connect the app to your brick and open the view that shows the connected devices --- you will see a live number for each motor.

On the brick, use the arrow buttons to reach the **Brick Apps** tab and open **Port View**. It shows a live number for every port. Arrow over to a wheel motor's port. Then **turn that wheel with your hand**, in the direction that would drive the robot *forward*, and watch its number change on the screen.
...
rows:
  - class: trial
    cells:
      - text: "B"
      - key: p3_spin_a_num
        aria: "Port B number direction"
      - key: p3_spin_a_dir
        aria: "Port B forward direction"
  - class: trial
    cells:
      - text: "C"
      - key: p3_spin_e_num
        aria: "Port C number direction"
      - key: p3_spin_e_dir
        aria: "Port C forward direction"
{{< /gridtable >}}
...
The brick is *counting* how far each wheel has turned, all the time, even when no program is running. Remember that it can. In a much later project, that counting is how you will drive exact distances.
{{< /callout >}}
...
| start motor B ↻ | **Starts and lets go.** The motor switches on and the very next block runs immediately --- while the motor keeps spinning, until something tells it to stop. |
| stop motor B | The something. Switches that motor off. |

### Why you always need the wait

Here is the thing that surprises everyone. Look at this program:

{{< wordblocks aria="Stack: when program starts, start motor B, start motor C, stop motor B, stop motor C" >}}
rows:
  - block:
      category: events
      shape: hat
...
The brick switches both motors on, and then --- faster than you can blink --- reads the next blocks and switches them off again. Remember from Project 2: it moves through blocks far quicker than your eye can follow.
...
| Code / part | What it means |
| --- | --- |
| start motor B ↺ | Left wheel on. Keep going. |
| start motor C ↻ | Right wheel on. Keep going. |
| wait 2 seconds | The motors are still running this whole time. |
| stop motor B · stop motor C | Now everything stops. |
...
{{< checklist >}}
- key: p3_can_ports
  label: "I can tell the motor ports from the sensor ports, and I know a motor works in any lettered port"
- key: p3_can_spin
