...
weight: 9
nav: "introductory"
mission_id: "introductory_ev3_coding_09"
mission_title: "Introductory Project 9 (EV3) — Names for Your Numbers"
...
eyebrow: "Introductory · EV3 Coding Project 9"
...
rows:
  ...
  - class: trial
    cells:
      - text: "My arm's \"up\" trip, in degrees from home"
      - key: p1_count_down
        aria: "Count up trip"
  - class: trial
    cells:
      - text: "My arm's \"horizontal\" trip, in degrees from home"
      - key: p1_count_open
        aria: "Count horizontal trip"
  - class: trial
    cells:
      - text: "My claw's \"closed on a cube\" trip, in degrees from open"
      - key: p1_count_closed
        aria: "Count closed"
  - class: trial
    cells:
...
It grips better now. But the closed number is different --- the claw now grips a cube **30 degrees sooner** than it used to.
...
{{< wordblocks aria="Before: run D clockwise for 55 degrees. After: run D clockwise for claw closed degrees" >}}
rows:
  - block:
      category: motors
      parts:
...
Open your `Arm` Wave program from Project 7. Make variables for your up, horizontal, and up-minus-horizontal trips, set them at the top, then drop the ovals into the run-for blocks in place of the numbers.
...
Same problem as Try It. Your claw's closed number drops by 30. Change it.
...
Small experiment. Arm at home first --- and keep a hand near the stop button, because the hops get bigger. Build this and watch the arm.

{{< wordblocks aria="Set swing to 30, run for swing degrees, change swing by 30, run again, change again, run again" >}}
rows:
  - block:
      category: events
      shape: hat
  ...
  - block:
      ...
      parts:
        ...
        - slot:
            kind: dropdown
            text: "degrees"
      note: "a 90° hop --- careful, is that past the top?"
{{< /wordblocks >}}

{{< ask key="p3_thirty_hops" label="Thirty hops" >}}Each hop was bigger than the last, because the variable grew between runs. What would you change to make thirty hops that are all the *same* size?{{< /ask >}}
...
rows:
  - class: trial
    cells:
      - text: "run A ↻ for (arm down) degrees"
      - key: p4_read_1
        aria: "Read one"
  - class: trial
    cells:
