...
description: "Drive out of the starting box in real inches, stop where a judge can see it, and return fully within the box. Fixing drift with the steering dial."
weight: 4
nav: "introductory"
mission_id: "introductory_spike_coding_04"
mission_title: "Introductory Project 4 (SPIKE) — Out and Back"
...
eyebrow: "Introductory · SPIKE Coding Project 4"
...
meta:
  ...
  - term: "Before You Start"
    definition: "Project 3 — wheel motors plugged into ports A (left) and E (right), the hub connected to the SPIKE App, and you can run a program that moves the robot."
---
...
In the SPIKE App you do not drive each wheel on its own. A **movement block** drives both wheels together as one matched pair --- the hub keeps the two motors in step for you.

### Meet the blocks

Every driving program in this project starts the same way: a hat block that starts the program, one block that tells the hub *which two motors* are the wheels, and one that sets how fast they go.
  ...
  - block:
      ...
      parts:
        ...
        - slot:
            kind: dropdown
            text: "A+E"
      note: "A is the left wheel, E is the right"
  - block:
      category: movement
      parts:
...
The colors match the SPIKE App, so you can find each block by its color: **yellow = Events**, **pink = Movement**, **orange = Control**.
{.muted}
...
{{< wordblocks aria="Word block: move, steering zero, for 24 inches" >}}
rows:
  - block:
      category: movement
      parts:
...
- The last part is the **unit**. We will use inches, to match your tape measure. The dropdown also offers cm, rotations, degrees, and seconds --- leave it on inches for now.

Steering 0 *should* mean perfectly straight. Here is the part nobody warns you about: **on a real robot, it usually almost does --- but not quite.** The hub keeps the motors matched, but it cannot fix things that are not motors:
...
{{< wordblocks aria="Two move blocks: move steering 0 for 24 inches forward, move steering 0 for negative 24 inches backward" >}}
rows:
  - block:
      category: movement
      parts:
...
{{< wordblocks aria="Full program: when program starts, set movement motors A plus E, set movement speed 50 percent, move steering 0 for 24 inches, wait 3 seconds, move steering 0 for negative 24 inches" >}}
rows:
  - block:
      category: events
      shape: hat
...
Your distances will not be 24 --- they will be whatever *your* field measurements say. That is what the next phase is for.
{.muted}
...
Start from your tape measure: put the distance you measured into the move block and run it. Watch where the robot *really* stops. Short? Add an inch. Past the zone? Take one away.

{{< gridtable >}}
columns:
- head: "Try"
- head: "Distance in the move block (in.)"
...
Movement blocks keep the wheels matched, so a SPIKE robot drifts less than most --- but less is not never. Watch which way your robot pulls as it drives out. Then nudge the steering number --- 1 or 2 at a time, no more.
...
Now add the return trip: one more move block, with a minus sign on the distance. Keep the *same* steering number --- the minus goes on the distance, never on the steering.
...
{{< ask key="p3_back_dist_diff" label="Backward distance difference" >}}Did you have to make the backward distance different from the forward distance? Why might that happen?{{< /ask >}}
...
rows:
  ...
  - cells:
      ...
      - key: p4_num_speed
        aria: "Movement speed"
  - cells:
      - text: "Distance to the zone (in.)"
      - key: p4_num_out_in
        aria: "Out distance"
  - cells:
      - text: "Distance back to the box (in.)"
      - key: p4_num_back_in
        aria: "Back distance"
  - cells:
