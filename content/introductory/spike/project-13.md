...
weight: 13
nav: "introductory"
mission_id: "introductory_spike_coding_13"
mission_title: "Introductory Project 13 (SPIKE) — Deciding What to Do"
...
eyebrow: "Introductory · SPIKE Coding Project 13"
...
{{< wordblocks aria="if F is pressed then play beep" >}}
rows:
  - control:
      category: control
      head:
...
{{< wordblocks aria="if D position greater than claw closed plus 10 then the claw is empty" >}}
rows:
  - control:
      category: control
      head:
      ...
      body:
        - block:
            category: display
            parts:
              - text: "turn on a sad face --- I grabbed air"
{{< /wordblocks >}}

The claw motor's own position, which the hub has been counting since Project 3, just became a sensor. Whether your empty claw ends up at a *bigger* or *smaller* number than your grip point depends on which way your claw closes --- check yours by hand and flip the hexagon if needed.
{.muted}
...
New project called `Decisions`. Build: *if (F is pressed?) then --- play a beep*. Run it twice --- once holding the sensor pressed, once not.
...
Replace the *if* with *wait until (F is pressed?)*, beep after it. Run it without pressing.
...
Open your `grab` block from Project 12. After the claw closes, add the empty-claw check from Learn It: if the claw position is past *claw closed* by more than about 10, show a sad face and stop; else, carry on.
...
- key: p4_can_empty
  label: "My robot can notice an empty claw using the claw motor's own position"
- key: p4_can_pick
