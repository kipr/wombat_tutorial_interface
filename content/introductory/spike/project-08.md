...
weight: 8
nav: "introductory"
mission_id: "introductory_spike_coding_08"
mission_title: "Introductory Project 8 (SPIKE) — Arm and Claw Together"
...
eyebrow: "Introductory · SPIKE Coding Project 8"
...
meta:
  ...
  - term: "Before You Start"
    definition: "Project 7 — and your claw must be built, with the claw motor plugged into port D . [Open the Claw Build Guide](/introductory/spike/builds/claw) — *Build guide coming soon — ask your teacher if this link is not live yet*"
---
## Try It --- Be the Robot
...
| --- | --- |
| arm = C | up = ___  ·  horizontal = ___  ·  down = ___ |
| claw = D | open = ___  ·  closed = ___ |

Notice the first entry of each group is the *[[PORT|port]]*, not a position. You now have seven numbers to keep straight --- and every one of them is typed into some block, somewhere in your program.
{.muted}

### Start in a safe shape

Project 7 gave you the arm rules --- gentle speed first, routes chosen on purpose. With two motors those rules matter twice as much: an arm that slams down *and* a claw that snaps shut is a good way to break something.

So every claw program starts with the same four blocks:

{{< wordblocks aria="Stack: set speed C 20, set speed D 20, run C to arm-up position, run D to claw-open position" >}}
rows:
  - block:
      category: events
      shape: hat
...
Speeds set, arm up, claw open. The robot begins every run in a known, safe shape --- every single time.
...
Same trick as Project 7: move it gently by hand and read the live number in the app. Your arm numbers are already on your card --- now add the claw.
...
"Closed" is whatever number grips the cube --- not the furthest the claw can reach. Send the claw past where the cube stops it and the motor strains against the cube --- while the block waits forever for an arrival that cannot come. Find the number that just grips, and stay there.
{{< /safety >}}
