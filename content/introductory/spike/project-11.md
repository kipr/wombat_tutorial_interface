...
description: "Degrees per inch behind the move block — and the yaw angle that feels the whole robot turn. Correct the overshoot, and the square finally closes."
weight: 11
nav: "introductory"
mission_id: "introductory_spike_coding_11"
mission_title: "Introductory Project 11 (SPIKE) — Counting and Turning by Feel"
...
eyebrow: "Introductory · SPIKE Coding Project 11"
...
credit: "KIPR · Botball Explorer — Introductory Projects · © KISS Institute for Practical Robotics 1997–2026"
...
Back in Project 3 you turned a wheel with your hand and watched a number change in the app. You were told to remember it. Here it is.

Open the device view. Do not run anything --- just roll a wheel with your hand and watch its number.
...
Since Project 3, your motors have been keeping track of exactly how far they turned --- in **degrees**. Every *move for inches* block you have ever run was secretly this count, plus one piece of arithmetic. Today you learn the arithmetic.
{{< /callout >}}
...
2. Note the wheel's number in the app (or roll it to a nice round number).
...
Project 10's checking watched a switch. The exact same checking can watch a **number** --- and the hub hands you two of them as round reporter ovals that fit into any hexagon.

| Code / part | What it means |
| --- | --- |
| A degrees counted | How far that wheel has rolled since its count was reset. This is the counter from Try It. |
| yaw angle | How far the *whole robot* has turned. Not the wheels --- the robot. There is a sense inside the hub that feels rotation, the way your inner ear does when you spin with your eyes shut. |

The wheel-degrees reporter lives in the motor extension of the block shelf --- if you do not see it, open **Extensions** at the bottom of the shelf and add the extra motor blocks.
{.muted}
...
{{< wordblocks aria="Project 10: wait until pressed. Project 11: wait until yaw greater than 89" >}}
rows:
  - block:
      category: control
      parts:
...
In Project 5 you tuned a 90° turn by trial and error, because wheel degrees are not robot degrees. The yaw angle *is* robot degrees. Here is the turn, rebuilt:

{{< wordblocks aria="set yaw to 0, start moving steering 100, wait until yaw greater than 89, stop moving" >}}
rows:
  - block:
      category: events
      shape: hat
...
A force sensor only helps where there is something to touch. The yaw angle works **anywhere on the field** --- open floor, mid-run, anywhere. And unlike wheel counting, it does not care if a wheel slipped. It feels the robot itself.
{{< /callout >}}

### It will go too far. Every time.

Run that turn and then look at the yaw angle in the app. You asked to stop past 89. You will see something like **96**.
...
Turning right, the yaw angle climbs. Turning **left**, it falls --- through zero and into negative numbers. That flips your hexagon:

| Code / part | What it means |
| --- | --- |
| right turn | steering 100  ·  wait until yaw **>** 83 |
| left turn | steering −100  ·  wait until yaw **<** −83 |

{{< safety title="⚠ Get the Direction Right or the Checking Never Ends" >}}
Spin left while waiting for yaw **>** 83 and you have a check that can never succeed --- the number is falling, away from 83, forever, while the robot spins on the spot like a figure skater who forgot the rest of the routine.
...
New project called `Gyro Turn`. Build the Learn It turn --- reset yaw, spin at steering 100, wait until past 89, stop. Run it, then read the final yaw angle in the app.
...
- head: "Where the yaw actually landed"
rows:
  - class: trial
    cells:
      - text: "1"
...
In Project 5 you drove a square with four wheel-counted turns and it did not close. Do it again --- same square, distances with *move for inches*, but every corner turned by yaw.
...
{{< ask key="p3_why_closes" label="Why it closes" >}}Your wheel-counted square drifted a little more with every corner. Why does a yaw-counted square not do that?{{< /ask >}}
...
rows:
  ...
  - cells:
      ...
      - key: p4_turn_speed
        aria: "Turn speed"
  - cells:
      - text: "My yaw overshoot at that speed"
      - key: p4_yaw_ov
        aria: "Yaw overshoot"
  - cells:
...
{{< checklist >}}
- key: p4_can_read
  label: "I can read a wheel's degree count and the yaw angle live in the app"
- key: p4_can_dpi
  label: "I can work out my robot's degrees per inch and convert a distance"
- key: p4_can_reset
  label: "I always reset the yaw angle before I start counting a turn"
- key: p4_can_ov
...
{{< ask key="p4_battery" label="Battery question" >}}Your battery is half flat, so the robot turns slower. What happens to a turn tuned by wheel degrees in Project 5? What happens to a turn that waits for the yaw angle?{{< /ask >}}
