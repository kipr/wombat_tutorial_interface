...
short_title: "Python 4.4"
hub_unit: 4
description: "Accumulating state and f-string logging — a full red-cube run that stacks, docks, and tracks its own points."
weight: 220
nav: python
track: python
type: labs
...
meta:
  ...
  - term: "CS1 Concepts"
    definition: "Accumulating [[VARIABLE|Variables]] · State · Console Output (f-strings) · Integration"
  - term: "Game Context"
...
{{< ask key="p2_printf" label="printf meaning" >}}What does the `{score}` inside an f-string do? Why is printing a report after each maneuver useful during a competition run?{{< /ask >}}
...
- Right now your score report prints as several separate `print` lines. Build one combined summary [[STRING|string]] with an f-string — for example, your score and maneuver count in a single sentence — before printing it in one call.
