...
meta:
  ...
  - term: "CS1 Concepts"
    definition: "else if Chains · Multi-Branch [[DECISION|Decisions]] · Linear [[FUNCTION|Functions]] in Code (y = mx + b) · Reliability-Oriented Motor Control"
  - term: "Game Context"
...
- Explain what an `else if` chain does and when it's the right tool versus separate [[IF STATEMENT|if statements]].
...
{{< ask key="p2_elseif" label="else if concept" >}}In your own words: what does an else if chain do differently from writing separate if statements? Give one example (robot or not) where else if is the right tool, and one where it would cause a bug.{{< /ask >}}
...
{{< ask key="p7_q1_elseif" label="Reflection 1" n=1 >}}What does an else if chain do that separate if statements don't, and why did the three speed zones need one?{{< /ask >}}
