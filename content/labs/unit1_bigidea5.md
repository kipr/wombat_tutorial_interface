...
A music app's "play" button is composed of smaller behaviors: find the file, decode it, send sound to the speaker, update the screen. Nobody rewrites "send sound to the speaker" for every song — it's a reusable block, called whenever it's needed.
{{% /callout %}}
...
Your pom-pushing run looked complex, but it was built from two or three simple behaviors reused in different orders. Every large intelligent system works this way: a self-driving car combines "stay in lane," "keep distance," and "obey signals" into a full trip; a warehouse robot combines "navigate," "grab," and "place" into an entire shift. No one writes a single giant behavior for "drive across the country." They compose it from small, tested parts.
