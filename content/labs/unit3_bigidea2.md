...
- code: |
    ...
    set_servo_position(0, target_position);   // move the final tick if needed
- text: |
...
void move_arm(int target_position)
{
	...
	set_servo_position(0, target_position);   // finish at the exact target
}
...
void move_claw(int target_position)
{
	...
	set_servo_position(3, target_position);   // finish at the exact target
}
{{< /code >}}
