...
- code: |
    ...
    motor(1, -50);   // right wheel backward  -> robot pivots RIGHT
- text: |
...
void turn_right() {
    ...
    while (gmpc(0) < RIGHT_TICKS) {    // left wheel rolls forward as we pivot right
        motor(0,  50);                // left wheel forward
        motor(1, -50);                // right wheel backward
    }
...
void turn_left() {
    ...
    while (gmpc(1) < LEFT_TICKS) {    // right wheel rolls forward as we pivot left
        ...
        motor(1,  50);               // right wheel forward
    }
    motor(0, 0); motor(1, 0); msleep(50);   // brake
}
{{< /code >}}
...
Your tuned turns are tools you'll use in every mission — so they belong in your library. Add `turn_left` and `turn_right` (and your `LEFT_TICKS`/`RIGHT_TICKS` values) to your `yourname.h` from last lab, fully commented.
...
void turn_right() {
    cmpc(0);
    while (gmpc(0) < RIGHT_TICKS) { motor(0, 50); motor(1, -50); }
...
void turn_left() {
    cmpc(1);
    while (gmpc(1) < LEFT_TICKS) { motor(0, -50); motor(1, 50); }
    motor(0, 0); motor(1, 0); msleep(50);
}
{{< /code >}}
...
{{< ask key="p7_q1_pivot" label="Reflection 1" n=1 >}}How does a robot pivot in place? Why does a right turn watch `gmpc(0)` but a left turn watch `gmpc(1)`?{{< /ask >}}
