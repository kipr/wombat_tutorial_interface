...
- text: |
    So far your functions ran the same way every time. A **parameter** is the named input in a function's prototype or definition. An **argument** is the value you supply when you call the function.
- code: |
    void Tick_Drive(int @@ticks@@);   // PARAMETER: the named input is ticks
...
You've built functions that always did exactly the same thing. In `void Tick_Drive(int ticks)`, `ticks` is the *parameter*: the named input in the function. In `Tick_Drive(2000)`, `2000` is the *argument*: the value supplied at that call. `Tick_Drive(2000)` and `Tick_Drive(1000)` call the same function with different arguments, so it drives two different distances.
...
{{< ask key="p2_argument_meaning" label="Parameters and arguments" >}}In your own words: what is the parameter in `void Tick_Drive(int ticks)`, and what is the argument in `Tick_Drive(2000)`? How does changing the argument change what the function does?{{< /ask >}}
...
Type this program. Notice that `int ticks` is the parameter in the `Tick_Drive` prototype and definition. The value in a call such as `Tick_Drive(2000)` is the argument. [[PROTOTYPE|Prototype]] above `main()`, definition below, as always.
...
void Tick_Drive(int @@ticks@@);   // PROTOTYPE: ticks is the parameter
...
void Tick_Drive(int @@ticks@@)       // DEFINITION: ticks receives the argument
{
...
- `ticks` is the parameter in `(int ticks)` in both the prototype and the definition
...
{{< ask key="p7_q2_argument" label="Reflection 2" n=2 >}}What is the difference between a *parameter* and an *argument*? Identify each one in `void Tick_Drive(int ticks)` and `Tick_Drive(2000)`, then explain how one function can drive many distances.{{< /ask >}}
