...
Write your run in `main()`, translating your plan into library calls. Start with the backward touch, then follow your path --- and put a `square_up()` at each reset checkpoint you planned. Scoring is optional here; you can [[PRINTF|printf]] milestones if you like, but the real goal is a **reliable, accurate run**.
...
- Use `graphics_line()` to plot 2-3 points of a [[SENSOR|sensor]] reading over time as connected line segments --- a simple live trend graph.
...
- `graphics_blit_enc()` can display a real bitmap image --- a small [[ARRAY|array]] of pixel color values --- instead of shapes. What's the difference between drawing a rectangle with `graphics_rectangle_fill` (a formula the computer calculates) and displaying a bitmap with `graphics_blit_enc` (stored pixel-by-pixel data)?
