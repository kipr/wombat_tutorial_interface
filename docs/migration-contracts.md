...
`data/introductory-legacy-inventory.json` is the machine-readable frozen
Introductory baseline. The frozen inventory was created from legacy HTML. Treat it
as immutable evidence, not as expected output to update casually.

Intentional contract changes belong in
`data/introductory-migration-exceptions.json`. Keep each exception narrow: identify
...
`node tools/introductory-importer/import.js`, which reads `tbc/` HTML through
Pandoc `html+raw_html` and the Lua writer at
`tools/introductory-importer/writer.lua`. Generated Markdown under
`content/introductory/ev3/` and `content/introductory/spike/` is the authored source
